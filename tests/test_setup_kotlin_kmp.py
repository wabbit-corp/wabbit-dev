...
def _managed_gradle_template(body: str) -> str:
    return "// Generated by app-wabbit-dev setup. Do not edit by hand.\n//\n" + body


def _managed_properties_template(body: str) -> str:
    return "# Generated by app-wabbit-dev setup. Do not edit by hand.\n#\n" + body


def _strip_managed_banner(text: str) -> str:
    lines = text.splitlines()
    if not lines:
        return ""

    first_line = lines[0]
    if not first_line.startswith(("// Generated by app-wabbit-dev setup.", "# Generated by app-wabbit-dev setup.")):
        return text.strip()

    comment_prefix = "//" if first_line.startswith("//") else "#"
    index = 0
...
def _make_context(
    ...
    *,
    ...
    return _Context(
        ...
        settings_template=jinja2.Template(_managed_gradle_template("")),
        subproject_settings_template=jinja2.Template(_managed_gradle_template("")),
        gitignore_template=jinja2.Template("# base"),
        gradle_gitignore_template=jinja2.Template("# gradle"),
        gradle_properties_template=jinja2.Template(_managed_properties_template("org.gradle.caching=true")),
    )
...
def test_setup_gradle_project_uses_jvm_template_for_jvm_only_platforms(
    ...
    assert "managed-integrity-v1:" in build_text
...
def test_setup_gradle_project_inlines_configured_build_inline_script(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_setup_side_effects(monkeypatch)
    ...
    ctx = _make_context(
        tmp_path,
        jvm_template="{% for inline_import in inline_extra_build_imports %}import {{ inline_import }}\n{% endfor %}before\n{{ inline_extra_build_script }}\nafter",
        kmp_template="KMP_TEMPLATE",
    )
...
def test_setup_gradle_project_always_renders_context_parameters_flag(
    ...
    plain_text = _strip_managed_banner((plain_project.path / "build.gradle.kts").read_text(encoding="utf-8"))
    ...
    kmp_text = _strip_managed_banner((kmp_project.path / "build.gradle.kts").read_text(encoding="utf-8"))
...
def test_setup_gradle_project_renders_google_repository_for_js_compose_projects(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_setup_side_effects(monkeypatch)
    project = _make_project(tmp_path / "kmp-site-proj", platforms=["js"])
    project.path.mkdir(parents=True, exist_ok=True)
    project.targets = [
        GradleTargetSpec(kind="js", browser=True),
    ]
    project.resolved_features = {
        "kotlin-compose-plugin": KotlinComposePlugin(),
    }
    project.resolved_dependencies = [
        Dependency(
            scope=None,
            target=MavenDependencyTarget(artifact="org.jetbrains.compose.runtime:runtime:1.9.1"),
        )
    ]
    ...
    build_text = (project.path / "build.gradle.kts").read_text(encoding="utf-8")
    assert "repositories {" in build_text
    assert "google()" in build_text
    assert "js {" in build_text
...
def test_setup_gradle_project_writes_prod_shaped_settings_even_in_local_mode(
    ...
    project.path.mkdir(parents=True, exist_ok=True)
    ctx = _make_context(tmp_path, jvm_template="JVM_TEMPLATE", kmp_template="KMP_TEMPLATE")
    ctx.subproject_settings_template = jinja2.Template(_managed_gradle_template("SETTINGS={{ project_name }}"))

    setup_gradle_project(ctx, project, interactive=False)

    settings_text = _strip_managed_banner((project.path / "settings.gradle.kts").read_text(encoding="utf-8"))
...
def test_setup_gradle_project_renders_dokka_source_link_for_standalone_jvm_repo(
    ...
    build_text = _strip_managed_banner((project.path / "build.gradle.kts").read_text(encoding="utf-8"))
...
def test_setup_gradle_project_generates_maven_central_and_docs_workflows_for_standalone_repo(
    ...
    monkeypatch.setattr(repo_docs_module, "repo_docs_workflows_owned_by_repo", lambda _config, _project: False)
...
def test_setup_gradle_project_skips_docs_workflows_when_repo_docs_own_them(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_setup_side_effects(monkeypatch)
...
def test_write_gradle_repo_root_workflows_skips_docs_when_repo_docs_own_them(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import dev.repo_docs as repo_docs_module
    ...
    docs_project = _make_repo_gradle_project(
        repo_root / "library",
    ...
    docs_project.path.mkdir(parents=True, exist_ok=True)
    docs_project.docs_enabled = True
    docs_project.docs_system = "dokka"
...
def test_write_gradle_repo_root_workflows_handles_compiler_plugin_repo_projects(tmp_path: Path) -> None:
    repo_root = tmp_path / "typeclasses"
    library_project = _make_repo_gradle_project(
        repo_root / "library",
        project_id="typeclasses/library",
        repo_root=repo_root,
        gradle_project_name="typeclasses-library",
        github_repo="wabbit-corp/typeclasses",
    )
    compiler_project = _make_repo_gradle_project(
        repo_root / "compiler-plugin",
        project_id="typeclasses/compiler-plugin",
        repo_root=repo_root,
        gradle_project_name="typeclasses-compiler-plugin",
        github_repo="wabbit-corp/typeclasses",
    )
    for project in (library_project, compiler_project):
        project.path.mkdir(parents=True, exist_ok=True)
        project.gradle_root = repo_root
        project.publish = True
        project.publish_target = "maven-central"
    ...
    compiler_project.resolved_features["kotlin-compiler-plugin"] = KotlinCompilerPlugin()
    ...
    ctx.config.defined_repos["typeclasses"] = RepoDefinition(
        repo_id="typeclasses",
        path=repo_root,
        github_repo="wabbit-corp/typeclasses",
        gradle_root_project_name="typeclasses",
        jvm_policy=None,
        supported_kotlin_versions=["2.2.20"],
        project_ids=["typeclasses/library", "typeclasses/compiler-plugin"],
    )
    ctx.gradle_compiler_plugin_release_publish_workflow_template = jinja2.Template("compiler release\n")
    ctx.gradle_compiler_plugin_snapshot_publish_workflow_template = jinja2.Template("compiler snapshot\n")

    write_gradle_repo_root_workflows(
        ctx,
        root_path=repo_root,
        repo_github_repo="wabbit-corp/typeclasses",
        projects=[library_project, compiler_project],
        docs_project=None,
        java_version=21,
    )
...
def test_setup_gradle_project_renders_kotlin_gradle_plugin_library_defaults(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_setup_side_effects(monkeypatch)
    project = _make_project(tmp_path / "kotlin-gradle-plugin-common", platforms=["jvm"])
    project.path.mkdir(parents=True, exist_ok=True)
    project.resolved_features = {
        "kotlin-gradle-plugin-library": KotlinGradlePluginLibrary(),
        "kotlin": Kotlin(),
        "jvm": Jvm(),
    }
    ...
    build_text = (project.path / "build.gradle.kts").read_text(encoding="utf-8")
    assert 'compileOnly(gradleApi())' in build_text
    assert 'compileOnly("org.jetbrains.kotlin:kotlin-gradle-plugin:2.2.20")' in build_text
    assert 'testImplementation("org.jetbrains.kotlin:kotlin-gradle-plugin:2.2.20")' in build_text
    assert "testImplementation(gradleTestKit())" in build_text
    assert "`java-gradle-plugin`" not in build_text


def test_setup_gradle_project_renders_intellij_platform_library_defaults(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_setup_side_effects(monkeypatch)
    project = _make_project(tmp_path / "kotlin-ij-plugin-common", platforms=["jvm"])
    project.path.mkdir(parents=True, exist_ok=True)
    project.resolved_features = {
...
def test_setup_gradle_project_uses_configured_paper_versions(
    ...
    ctx.subproject_settings_template = jinja2.Template(
        _managed_gradle_template("PLUGIN={{ paperweight_userdev_plugin_version }}")
    )
