...
from dev.config import IntellijPlatformLibrary, IntellijPlugin
...
def test_renderable_gradle_features_preserves_intellij_platform_library_metadata() -> None:
    rendered = _renderable_gradle_features(
        {
            "intellij-platform-library": IntellijPlatformLibrary(
                ideaVersion="2025.3",
    ...
    feature = rendered["intellij-platform-library"]
    assert isinstance(feature, IntellijPlatformLibrary)
    assert feature.ideaVersion == "2025.3"
    assert feature.bundledPlugins == ["org.jetbrains.kotlin"]
