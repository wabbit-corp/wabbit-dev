...
def test_setup_python_project_generates_docs_and_workflows(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (project.path / ".github" / "workflows" / "release-publish.yml").is_file()
...
def test_setup_python_project_uses_repo_docs_subpath_and_skips_project_workflows_for_nested_repo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    repo_root = tmp_path / "sem"
    project = _make_python_project(repo_root / "python-lang-mu", github_repo="wabbit-corp/sem")
    project.name = "python-lang-mu"
    project.project_id = "sem/python-lang-mu"
    project.repo_id = "sem"
    project.repo_root = repo_root
    project.path.mkdir(parents=True, exist_ok=True)
    (project.path / "README.md").write_text("# python-lang-mu\n", encoding="utf-8")

    config = Config(raw=Document([]))
    config.defined_projects["sem/python-lang-mu"] = project
    config.defined_repos["sem"] = RepoDefinition(
        repo_id="sem",
        path=repo_root,
        github_repo="wabbit-corp/sem",
        gradle_root_project_name=None,
        jvm_policy=None,
        project_ids=["sem/python-lang-mu"],
    )
    ...
    mkdocs_text = (project.path / "mkdocs.yml").read_text(encoding="utf-8")
    assert "site_url: https://wabbit-corp.github.io/sem/python-lang-mu/" in mkdocs_text
    assert not (project.path / ".github" / "workflows" / "docs-quality.yml").exists()
    assert not (project.path / ".github" / "workflows" / "docs-deploy.yml").exists()


def test_write_repo_docs_root_generates_repo_docs_workflows_and_builder_script(tmp_path: Path) -> None:
    from mu.types import Document

    import dev.tasks.setup as setup_module
    from dev.config import Config, GradleProject, OwnershipType, RepoDefinition, Version

    repo_root = tmp_path / "kotlin-demo"
    repo_root.mkdir(parents=True, exist_ok=True)
    (repo_root / "README.md").write_text("# kotlin-demo\n", encoding="utf-8")
    (repo_root / ".github" / "workflows").mkdir(parents=True, exist_ok=True)
    (repo_root / ".github" / "workflows" / "docs-pages.yml").write_text("legacy\n", encoding="utf-8")

    library_project = GradleProject(
        ...
        quarantine=False,
        publish=True,
        github_repo="wabbit-corp/kotlin-demo",
        ownership=OwnershipType.WABBIT,
        raw_dependencies=[],
        raw_features=[],
        resolved_dependencies=[],
        ...
        project_id="kotlin-demo/library",
        repo_id="kotlin-demo",
        repo_root=repo_root,
        gradle_root=repo_root,
        gradle_project_name="kotlin-demo",
        docs_enabled=True,
        docs_system="dokka",
    ...
    config = Config(raw=Document([]))
    config.defined_projects["kotlin-demo/library"] = library_project
    config.defined_repos["kotlin-demo"] = RepoDefinition(
        repo_id="kotlin-demo",
        path=repo_root,
        github_repo="wabbit-corp/kotlin-demo",
        gradle_root_project_name="kotlin-demo",
        jvm_policy=None,
        project_ids=["kotlin-demo/library"],
    )

    ctx = _make_setup_context("")
    ctx.config = config

    written_root = setup_module._write_repo_docs_root(ctx, "kotlin-demo")

    assert written_root == str(repo_root.resolve())
    assert (repo_root / ".github" / "workflows" / "docs-quality.yml").read_text(encoding="utf-8") == (
        "name: Repo Docs Quality\n"
    )
    assert (repo_root / ".github" / "workflows" / "docs-deploy.yml").read_text(encoding="utf-8") == (
        "name: Repo Docs Deploy\n"
    )
    builder_script = (repo_root / "scripts" / "build_repo_docs_site.py").read_text(encoding="utf-8")
    assert builder_script.startswith("# Generated by app-wabbit-dev setup. Do not edit by hand.\n")
    assert "managed-integrity-v1:" in builder_script
    assert not (repo_root / ".github" / "workflows" / "docs-pages.yml").exists()


def test_write_standalone_gradle_repo_docs_root_generates_repo_docs_bundle(tmp_path: Path) -> None:
    import dev.tasks.setup as setup_module
    from dev.config import GradleProject, OwnershipType, Version

    project_root = tmp_path / "kotlin-base58"
    project_root.mkdir(parents=True, exist_ok=True)
    (project_root / "README.md").write_text("# kotlin-base58\n", encoding="utf-8")
    (project_root / "docs").mkdir(parents=True, exist_ok=True)
    (project_root / "docs" / "dokka-module.md").write_text("# Module\n", encoding="utf-8")
    ...
    project = GradleProject(
        ...
        name="kotlin-base58",
        version=Version.parse("0.0.1"),
        description="Base58 for Kotlin.",
        authors=[],
        license="AGPL",
        quarantine=False,
        publish=True,
        github_repo="wabbit-corp/kotlin-base58",
        ownership=OwnershipType.WABBIT,
        raw_dependencies=[],
        raw_features=[],
        resolved_dependencies=[],
    ...
    written_root = setup_module._write_standalone_gradle_repo_docs_root(ctx, project)

    assert written_root == str(project_root.resolve())
    assert (project_root / ".github" / "workflows" / "docs-quality.yml").read_text(encoding="utf-8") == (
        "name: Repo Docs Quality\n"
    )
    assert (project_root / ".github" / "workflows" / "docs-deploy.yml").read_text(encoding="utf-8") == (
        "name: Repo Docs Deploy\n"
    )
    builder_script = (project_root / "scripts" / "build_repo_docs_site.py").read_text(encoding="utf-8")
    assert builder_script.startswith("# Generated by app-wabbit-dev setup. Do not edit by hand.\n")
    assert "managed-integrity-v1:" in builder_script
