...
def test_state_uses_local_config_files(tmp_path: Path) -> None:
...
def test_state_without_local_config_files_has_no_config_paths(tmp_path: Path) -> None:
    target_repo = tmp_path / "target"
    target_repo.mkdir()

    state = qa._get_state(target_repo)
    assert state.pyproject_config is None
    assert state.mypy_config is None
...
def test_coverage_report_skips_when_pytest_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def always_coverage_available(state: qa.PythonQaRepoState) -> bool:
        return True

    monkeypatch.setattr(qa, "_coverage_available", always_coverage_available)
...
def test_pytest_and_unittest_both_run(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def never_coverage_available(state: qa.PythonQaRepoState) -> bool:
        return False

    monkeypatch.setattr(qa, "_tool_executable", fake_tool_executable)
    monkeypatch.setattr(qa, "_coverage_available", never_coverage_available)
    monkeypatch.setattr(qa, "_run_subprocess", fake_run_subprocess)
...
def test_run_subprocess_merges_pytest_issues_with_same_root_cause(
    ...
    result = qa._run_subprocess(state, "pytest", ["pytest"])
    assert len(result.issues) == 1
    issue_data = result.issues[0].data
    assert issue_data is not None
    issue_message = issue_data.get("message", "")
    assert isinstance(issue_message, str)
    assert "dev.test_a" in issue_message
    assert "dev.test_b" in issue_message
...
def test_run_subprocess_merges_unittest_issues_with_same_reason(
    ...
    def fake_parse_issues(label: str, log_path: Path, fail_under: int) -> list[qa.UnittestIssue]:
        return [
            qa.UnittestIssue(
                outcome="ERROR",
...
def test_failure_issue_reports_signal_for_negative_return_code() -> None:
    issue = qa.failure_issue("basedpyright", "", rc=-9)
    assert isinstance(issue, qa.PyrightFailure)
    assert "signal 9" in issue.message
    assert "SIGKILL" in issue.message
...
def test_run_subprocess_failure_with_empty_output_includes_command(
    ...
    def fake_parse_issues(label: str, log_path: Path, fail_under: int) -> list[qa.ParsedIssue]:
        return []

    monkeypatch.setattr(qa, "parse_issues", fake_parse_issues)
