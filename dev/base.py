...
class Scope:
    ...
    def __exit__(self, exc_type: type[BaseException] | None, value: BaseException | None, traceback: TracebackType | None) -> None:
        for fn in self.deferred[::-1]:
            match fn:
                ...
                case OnFailureCallback(fn):
                    if exc_type is None: continue
                    try:
                        fn(exc_type)
                    except Exception as e:
                        error(f"Error during deferred execution: {e}")
                case OnSuccessCallback(fn):
                    if exc_type is not None: continue
                    try:
                        fn()
                    except Exception as e:
                        error(f"Error during deferred execution: {e}")
