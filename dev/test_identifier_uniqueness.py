...
def test_identifier_uniqueness_still_reports_duplicates(tmp_path: Path) -> None:
    ...
    duplicate = '"2ecbfb56-85d7-4e32-84cb-b2f175acf240"'  # check:ignore E_DUPLICATE_IDENTIFIER
...
def test_identifier_uniqueness_respects_inline_check_ignore(tmp_path: Path) -> None:
    project_path = tmp_path / "project"
    project_path.mkdir()

    duplicate = '"2ecbfb56-85d7-4e32-84cb-b2f175acf240"'  # check:ignore E_DUPLICATE_IDENTIFIER
    (project_path / "a.py").write_text(f"x = {duplicate}\n", encoding="utf-8")
    (project_path / "b.py").write_text(
        f"y = {duplicate}  # check:ignore E_DUPLICATE_IDENTIFIER\n",
        encoding="utf-8",
    )
