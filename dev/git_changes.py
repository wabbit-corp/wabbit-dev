...
def compute_repo_diffs(repo: Repo, include_untracked: bool = True) -> List[FileDiff]:
    ...
    for diff in staged_diff_list:
        ...
        file_diff = FileDiff(
            ...
            staged=True,
            unstaged=False,  # Mark as staged
            old_content_sha=a_sha,
            new_content_sha=b_sha,
    ...
    for diff in unstaged_diff_list:
        ...
        wt_exists = wt_content is not None or (
            wt_mode is not None and stat.S_ISLNK(wt_mode)
        )  # WT exists if content or symlink
        ...
        if path_key in diffs_dict:
            ...
            is_clearly_textual_hw = {head_type, wt_type} <= {
                FileType.TEXT,
                FileType.EMPTY,
            }
            ...
            if final_change_type != ChangeType.UNCHANGED:
                ...
                file_diff = FileDiff(
                    ...
                    old_content_sha=head_sha,
                    new_content_sha=wt_sha,
                    old_mode=head_mode,
                    new_mode=wt_mode,
                    old_type=head_type,
                    new_type=wt_type,
        ...
        for path in untracked_files:
            ...
            if path_key_exists:
                ...
                if existing_diff:
                    ...
                    logging.warning(
                        f"Path '{path}' listed as untracked and path_key_exists=True, but no diff found. Skipping."
                    )
                ...
                if wt_mode is not None and not stat.S_ISDIR(wt_mode):
                    ...
                    diffs_dict[path] = FileDiff(
                        ...
                        staged=False,
                        unstaged=True,
                        untracked=True,  # Set untracked=True
                        old_content_sha=None,
                        new_content_sha=wt_sha,
                        old_mode=None,
                        new_mode=wt_mode,
                        old_type=FileType.EMPTY,
                        new_type=wt_type,  # Assume old type is empty
    ...
    for path_key in list(diffs_dict.keys()):  # Iterate over copy of keys
        ...
        file_diff.unified_diff = _generate_diff_text(
            ...
            final_content,
            head_type,
            final_type,  # Use types derived from actual content/state
        )
        ...
        if current_change_type == ChangeType.DELETED:
            ...
            if file_diff.old_path is None:
                file_diff.old_path = current_path  # Fallback
            ...
            if file_diff.new_path is None:
                file_diff.new_path = current_path  # Fallback
...
class GitTestBase(unittest.TestCase):
    ...
    def _stage_remove(self, filename):
        ...
        try:
            ...
            if "did not match any files" in str(e.stderr):
                ...
                if filepath.exists():
                    logging.warning(
                        f"File {filepath} still exists after 'git rm -f' reported no match. Attempting unlink."
                    )
                    try:
                        filepath.unlink()
    ...
    def _assert_diff(
        ...
        if target_diff is None:
            diff_summary = [
                (
                    Path(d.path).as_posix() if d.path else "N/A",
                    d.change_type.name,
                    f"S:{d.staged}",
                    f"U:{d.unstaged}",
                    f"T:{d.untracked}",
        ...
        # Assertions
        self.assertEqual(
            target_diff.change_type,
            expected_type,
            f"Path {expected_path_key}: ChangeType mismatch ({target_diff.change_type.name} vs {expected_type.name})",
        )
        ...
        if expected_type == ChangeType.RENAMED:
            self.assertEqual(
                target_old_path_key,
                expected_old_path_key,
                f"Path {expected_path_key}: Old path mismatch for rename",
            )
            self.assertEqual(
                target_new_path_key,
                expected_path_key,
                f"Path {expected_path_key}: New path mismatch for rename",
            )
        elif expected_type == ChangeType.DELETED:
            self.assertEqual(
                target_old_path_key,
                expected_path_key,
                f"Path {expected_path_key}: Old path mismatch for delete",
            )
            self.assertIsNone(
                target_new_path_key,
                f"Path {expected_path_key}: New path should be None for delete",
            )
        elif expected_type == ChangeType.ADDED:
            self.assertIsNone(
                target_old_path_key,
                f"Path {expected_path_key}: Old path should be None for add",
            )
            self.assertEqual(
                target_new_path_key,
                expected_path_key,
                f"Path {expected_path_key}: New path mismatch for add",
            )
        else:  # MODIFIED, MODE_CHANGED, TYPE_CHANGED
            ...
            self.assertEqual(
                target_old_path_key,
                expected_path_key,
                f"Path {expected_path_key}: Old path mismatch for {expected_type.name}",
            )
            self.assertEqual(
                target_new_path_key,
                expected_path_key,
                f"Path {expected_path_key}: New path mismatch for {expected_type.name}",
            )
        ...
        self.assertEqual(
            target_diff.staged,
            expected_staged,
            f"Path {expected_path_key}: Staged flag mismatch",
        )
        self.assertEqual(
            target_diff.unstaged,
            expected_unstaged,
            f"Path {expected_path_key}: Unstaged flag mismatch",
        )
        self.assertEqual(
            target_diff.untracked,
            expected_untracked,
            f"Path {expected_path_key}: Untracked flag mismatch",
        )
        self.assertEqual(
            target_diff.partial_staging_suspected,
            expected_partial,
...
class TestGatherChangesEnhanced(GitTestBase):
    ...
    def test_90_committed_deleted_from_index_only(self):
        ...
        self.assertIsNone(
            d.unified_diff,
            "Expected no unified diff as HEAD content matches WT content",
        )
