...
class Cashier:
    ...
    def __init__(self, path: str):
        ...
        try:
            ...
            logger.error("Failed to initialize Cashier database at %s: %s", self.path, e, exc_info=True)
            ...
            if self._conn:
                ...
                try:
                    self._conn.close()
                     ...
                     logger.error("Error closing connection for %s: %s", self.path, e, exc_info=True)
                finally:
        ...
        with self._lock:
            ...
            if effective_ttl is not None:
                ...
                if (now - insert_time) > effective_ttl:
                    logger.debug("Key %s expired (insert=%.1f, now=%.1f, effective_ttl=%.1f). Deleting.",
                                 key, insert_time, now, effective_ttl)
    ...
    def set(
        ...
        logger.debug("Setting key in cache: %s (fqn=%s, db=%s, specific_ttl=%s)",
                     key, fqn, self.path, specific_ttl)
        now = time.time()
        ...
        with self._lock:
            ...
            with self._conn:
                ...
                if max_age is not None:
                    cutoff = now - max_age
                    logger.debug("Cleaning up items older than %.1f seconds for fqn=%s", max_age, fqn)
                    ...
                    if deleted_rows > 0:
                         logger.debug("Removed %d old items via max_age for fqn=%s", deleted_rows, fqn)
                ...
                if max_fqn_capacity is not None and max_fqn_capacity > 0:
                    ...
                    if count > max_fqn_capacity:
                        ...
                        logger.debug(
                            ...
                            count, max_fqn_capacity, to_remove, fqn
                        )
                        self._conn.execute(
                            self._REMOVE_OLDEST_FQN_SQL,
                            (fqn, to_remove)
                        )
...
def _cleanup_all_cashiers():
    ...
    for instance in instances_to_close:
        try:
            ...
            logger.error("atexit: Error closing Cashier for DB %s: %s", instance.path, e, exc_info=True)
...
def _build_cache_key(fqn, func_sig, args, kwargs, exclude_params):
    ...
    try:
        ...
        return key_hash
         ...
         logger.error("Failed to generate cache key for %s due to pickling error: %s", fqn, e, exc_info=True)
    ...
    def decorator(fn):
        ...
        try:
            cashier = get_cashier_instance(path=path)
             ...
             logger.critical("Failed to obtain Cashier instance for %s used by %s. Caching disabled for this function.", path, fqn, exc_info=True)
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                 ...
                 logger.error("Skipping cache due to key generation failure for %s", fqn, exc_info=True)
                 if is_async:
            ...
            try:
                ...
                logger.error("Cache get failed for key=%s (fqn=%s): %s. Calling function.", key, fqn, e, exc_info=True)
            ...
            def _execute_and_cache(result):
                ...
                if ttl_policy_func:
                    try:
                        ...
                        logger.warning("ttl_policy_func execution failed for fqn=%s: %s. Defaulting to standard TTL.",
                                     fqn, policy_exc, exc_info=True)
                        cache_directive = None # Default on policy error

                if cache_directive is NO_CACHE:
                    logger.debug("Skipping cache set based on ttl_policy_func result (NO_CACHE) for key=%s", key)
                    # Do not cache
                else:
                    ...
                    try:
                        ...
                        cashier.set(
                            key=key, fqn=fqn, val=pickled_result,
                            max_age=max_age, max_fqn_capacity=max_fqn_capacity,
                            specific_ttl=specific_ttl_to_set # Pass policy result here
                        )
                        logger.debug("Stored result in cache for key=%s (fqn=%s, specific_ttl=%s)",
                                     key, fqn, specific_ttl_to_set)
                    except Exception as cache_set_exc:
                        logger.error("Failed to cache result for key=%s (fqn=%s): %s",
                                     key, fqn, cache_set_exc, exc_info=True)
            ...
            if not is_async:
                ...
                try:
                    ...
                    logger.error("Original sync function execution failed for key=%s (fqn=%s): %s", key, fqn, e)
                ...
                async def async_executor():
                    try:
                        ...
                        logger.error("Original async function execution failed for key=%s (fqn=%s): %s", key, fqn, e)
                        raise # Propagate function error
        ...
        def clear_cache_for_this_fn():
            ...
            try:
                ...
                instance.delete_by_fqn(fqn)
                 ...
                 logger.error("Failed to clear cache for fqn=%s (path=%s): %s", fqn, path, e, exc_info=True)
                 # Optionally re-raise
        ...
        def close_cache_for_this_fn():
            ...
            """
            logger.warning("Closing cache connection for path %s (used by %s and potentially others).", path, fqn)
            try:
                instance = get_cashier_instance(path=path)
