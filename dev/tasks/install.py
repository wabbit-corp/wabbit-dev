...
from dev.commit_policy import git_hooks_path, git_root
...
from dev.repo_resolution import ResolvedRepoTarget, configured_repo_targets, inferred_repo_targets, resolve_repo_targets
...
class HooksInstallResult:
    results: tuple[HookInstallResult, ...]
...
def install_hooks(
    ...
    *,
    ...
    resolved_targets = _resolve_hook_targets(targets)
    results = tuple(_install_hooks_for_target(target) for target in resolved_targets)
    result = HooksInstallResult(results=results)
    if json_output:
        print(json.dumps(result.to_payload(), indent=2))
    else:
        _print_hooks_install_result(result)
    return result


def _resolve_hook_targets(targets: Sequence[str] | None) -> tuple[ResolvedRepoTarget, ...]:
    requested_targets = list(targets or [])
    if find_workspace_root() is not None:
        config = load_config()
        effective_targets = inferred_repo_targets(config, requested_targets)
        if effective_targets:
            return tuple(resolve_repo_targets(effective_targets, config=config))
        return tuple(configured_repo_targets(config))

    if requested_targets:
        resolved: list[ResolvedRepoTarget] = []
        for target in requested_targets:
            resolved.append(ResolvedRepoTarget(name=target, path=Path(target).expanduser()))
        return tuple(resolved)

    current_path = Path.cwd()
    return (ResolvedRepoTarget(name=current_path.name, path=current_path),)


def _install_hooks_for_target(target: ResolvedRepoTarget) -> HookInstallResult:
    try:
        repo_root = git_root(target.path)
    except ValueError as ex:
        fallback_root = target.path.resolve()
        hooks_path = fallback_root / ".git" / "wabbit-dev-hooks"
        hook_path = hooks_path / "commit-msg"
        return HookInstallResult(
            ...
            status="skipped",
            details=f"not a git repository: {ex}",
        )

    hooks_path = git_hooks_path(repo_root)
    hook_path = hooks_path / "commit-msg"
    hooks_path.mkdir(parents=True, exist_ok=True)
    _write_executable(hook_path, _commit_msg_hook_text())

    completed = _run_command(("git", "config", "core.hooksPath", str(hooks_path)), cwd=repo_root)
    if completed.returncode != 0:
        return HookInstallResult(
            name=target.name,
            repo_root=repo_root,
    ...
    return HookInstallResult(
        ...
        hooks_path=hooks_path,
        status="installed",
        details="commit-msg hook runs `dev commit verify --staged` before local commits",
    )


def _commit_msg_hook_text() -> str:
    return (
        "#!/bin/sh\n"
        "# Generated by dev install hooks. Do not edit by hand.\n"
        "exec dev commit verify --message-file \"$1\" --staged --quiet\n"
    )
...
def _print_hooks_install_result(result: HooksInstallResult) -> None:
    success("Git hook installation complete.")
    for hook_result in result.results:
        if hook_result.status == "failed":
            warning(f"{hook_result.name}: failed to install hooks: {hook_result.details}")
            continue
        if hook_result.status == "skipped":
            warning(f"{hook_result.name}: skipped hook install: {hook_result.details}")
            continue
        info(f"{hook_result.name}: commit-msg hook -> {hook_result.hook_path}")
        info(f"{hook_result.name}: git config core.hooksPath {command_text(hook_result.hooks_path)}")
