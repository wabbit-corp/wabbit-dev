...
def main():
    ...
    if os.path.exists(".entroignore"):
        with open(".entroignore", 'rt', encoding='utf-8') as f:
            try:
                ignore = f.readlines()
            except UnicodeDecodeError:
                print("UnicodeDecodeError: .entroignore")
                return
    ...
    parser.add_argument('-g', '--git', action="store_true", help='Search git list-files. Use with -d')
    ...
    if args.git:
        if args.directory:
            ...
            files = [file for file in files if file != ""] # remove empty lines

            # Use ignore list using globbing
            import fnmatch
            assert fnmatch.fnmatchcase('foo.txt', 'foo.txt')
            ...
            print("Error: -g can only be used with -d")
            return
            ...
            for string in base64_strings: # step through each b64 string
                ...
                if b64_entropy > b64_minimum: # if entropy is significant
                    ...
                    if verbose:
                        ...
                        p = filename + ":" + str(line_counter) + ": " + line
                    printable.append(p)
